...
public class OrderDAO {
    ...
    public static OrderBean getOrderByNumber(Connection con, String orderNr, int idCompany) throws SQLException {
        final String sql =
                "SELECT o.*, op.id AS op_id, op.price AS purchase_price, " +
                        "p.id AS p_id, p.name AS p_name, p.description AS p_desc, p.state AS p_state " +
                        "FROM order_table o " +
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    if (!rs.wasNull()) {
                        ...
                        if (rawState != null) {
                            product.setStatus(it.unisa.nexware.application.enums.ProductStatus.fromString(rawState.trim().toUpperCase()));
                        }
